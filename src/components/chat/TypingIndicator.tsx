...
export const TypingIndicator: React.FC<TypingIndicatorProps> = ({
  ...
  const getTypingText = () => {
    if (typingUsers.length === 1) {
      ...
      return (
        ...
        ' and ' +
        (typingUsers[1].userName || 'someone') +
        ' are typing...'
      );
    }
  ...
  const getRecordingText = () => {
    if (recordingUsers.length === 1) {
      ...
      return (
        (recordingUsers[0].userName || 'Someone') +
        ' and ' +
        (recordingUsers[1].userName || 'someone') +
        ' are recording...'
      );
    }
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      {recordingUsers.length > 0 ? (
        <>
          ...
          <Text style={[styles.text, { color: colors.textSecondary }]}>{getRecordingText()}</Text>
        </>
      ) : (
        <>
          <View style={styles.dotsContainer}>
            ...
            />
            <Animated.View
              style={[styles.dot, { backgroundColor: colors.primary, opacity: 0.6 }]}
            />
            <Animated.View
              style={[styles.dot, { backgroundColor: colors.primary, opacity: 0.8 }]}
            />
          </View>
